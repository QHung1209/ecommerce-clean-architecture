...
import { VerificationCode, VerificationCodeType } from '../../domain/entities/verification-code.entity';
...
describe('VerifyOtpUseCase', () => {
  ...
  it('should successfully verify OTP and delete it from DB', async () => {
    const vc = VerificationCode.createWithExpiration(Email.create(email), code, type, 5);
  ...
  it('should throw BadRequestException if code is not found', async () => {
    ...
    await expect(useCase.execute(email, code, type)).rejects.toThrow(BadRequestException);
  });

  it('should throw BadRequestException if code is wrong or expired', async () => {
    const vc = VerificationCode.createWithExpiration(Email.create(email), '999999', type, 5);
    repo.findByEmailAndType.mockResolvedValue(vc);
